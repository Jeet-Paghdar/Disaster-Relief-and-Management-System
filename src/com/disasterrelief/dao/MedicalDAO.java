...
public class MedicalDAO {
    ...
    public void addMedicalRecord(MedicalRecord record) throws SQLException {
        String sql = "INSERT INTO MEDICAL_RECORD (VICTIM_ID, BLOOD_TYPE, PRESCRIPTIONS, TREATMENT_DETAILS, TREATMENT_DATE, WORKER_ID) VALUES (?, ?, ?, ?, ?, ?)";

        try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {

            stmt.setInt(1, record.getVictimId());
            stmt.setString(2, record.getBloodType());
            stmt.setString(3, record.getPrescriptions());
            stmt.setString(4, record.getTreatmentDetails());

            // Handle null treatment dates for new records
            stmt.setDate(5, record.getTreatmentDate() != null ? Date.valueOf(record.getTreatmentDate()) : null);

            stmt.setInt(6, record.getWorkerId());
            stmt.executeUpdate();
        }
    }
    ...
    public List<MedicalRecord> getMedicalRecords(int victimId) throws SQLException {
        ...
        String sql = "SELECT * FROM MEDICAL_RECORD WHERE VICTIM_ID = ?";
